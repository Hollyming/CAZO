...
def validate_adapt(val_loader, model, args, writer):
    ...
    with torch.no_grad():
        ...
        for i, dl in enumerate(val_loader):
            ...
            current_loss = None
            if hasattr(model, 'final_loss'):  # for:FOA, CAZO, Tent, CoTTA,etc.
...
if __name__ == '__main__':
    ...
    # Define an auxiliary function to select the corresponding adapter wrapper based on 'arch'
    ...
    if args.algorithm == 'cazo':
        ...
        from tta_library.zo_base import ZO_Base  # import ZO_Base algorithm
        # use ZO_Base algorithm
        net = wrap_model_with_adapter(net, args)
        adapt_model = ZO_Base(
            model=net,
            fitness_lambda=args.fitness_lambda,
        ...
        adapt_model = net# strict validate_adapt
    else:
