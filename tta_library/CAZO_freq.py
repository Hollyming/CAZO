...
class CAZO_Freq(nn.Module):
    """
    CAZO_Freq: CAZO算法的变体，支持Hessian对角估计的间隔更新
    与CAZO的区别：D矩阵不是每步都更新，而是每τ步才更新一次
    这样可以研究更新频率对算法性能的影响
    """
    def __init__(self, model, fitness_lambda=0.4, lr=0.01, 
                 pertub=20, epsilon=0.1, optimizer_type='sgd', beta=0.9, nu=0.1, 
                 tau=1, use_pure_entropy=False):
        """
        Initialize CAZO_Freq algorithm
        ...
        Args:
            ...
            tau: update frequency for D matrix (D is updated every tau steps)
    ...
    def _sample_perturbations(self, num_samples, dim):
        ...
        if self.d_update_count == 0:
            # 如果D从未更新过，直接使用D
            H_diag = self.D
        else:
            # 使用公式: H̃_t = diag(D_t / (1 - (1 - ν)^(update_count)))
            # 这里的update_count是实际更新D的次数，而不是时间步t
            # H_diag = self.D / (1 - (1 - self.nu)**self.d_update_count)
    ...
    def forward(self, x):
        ...
        print(f"Start using zero-order method to estimate gradient (Step {self.t}, D updates: {self.d_update_count})...")
        ...
        # 关键修改：每τ步才更新D矩阵
        should_update_d = (self.t % self.tau == 0) or (self.t == 0)
        
        if self.prev_grad_estimate is not None and should_update_d:
            # D_t = (1 - ν)D_{t-1} + νĝ²(x_{t-1})
            self.D = (1 - self.nu) * self.D + self.nu * self.prev_grad_estimate**2
            self.d_update_count += 1
            print(f"*** D matrix updated at step {self.t} (total updates: {self.d_update_count}) ***")
        elif not should_update_d:
            print(f"D matrix NOT updated at step {self.t} (will update at step {(self.t // self.tau + 1) * self.tau})")
    ...
    def reset(self):
        ...
        self.prev_grad_estimate = None
        self.t = 0
        self.d_update_count = 0  # 重置D更新计数器
