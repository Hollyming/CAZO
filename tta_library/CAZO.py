...
class CAZO(nn.Module):
    ...
    def obtain_origin_stat(self, train_loader):
        ...
        if os.path.exists(save_path):
            print('===> loading pre-computed mean and variance from file')
            ...
            # 保存计算结果
            print('===> saving calculated mean and variance to file')
            torch.save({
        ...
        num_layers = len(self.model.vit.blocks)  # automatically detect number of layers, vit_base_patch16_224 has 12 layers
        head_dim = self.model.vit.blocks[0].attn.head_dim  # automatically detect head dimension, vit_base_patch16_224's head dimension is 64
        for _, m in self.model.vit.named_modules():  # iterate over all modules of ViT model
            if type(m) == PTQSLBatchingQuantMatMul:  # if type is PTQSLBatchingQuantMatMul
                m._get_padding_parameters(
                    torch.zeros((1,num_layers,197,head_dim)).cuda(),  # remove prompt tokens
                    torch.zeros((1,num_layers,64,197)).cuda()   # remove prompt tokens
                )
            elif type(m) == SoSPTQSLBatchingQuantMatMul:  # if type is SoSPTQSLBatchingQuantMatMul
                m._get_padding_parameters(
                    torch.zeros((1,num_layers,197,197)).cuda(),  # remove prompt tokens
                    torch.zeros((1,num_layers,197,head_dim)).cuda()    # remove prompt tokens
                )
