...
class ZO_Base(nn.Module):
    """
    ZO_Base: Basic Zero-Order Optimization algorithm

    This algorithm uses standard zero-order optimization method to estimate gradients through random perturbation sampling
    """
    def __init__(self, model: AdaFormerViT, fitness_lambda=0.4, lr=0.01, 
                 pertub=20, epsilon=0.1,
                 optimizer_type='sgd', beta=0.9):
        """
        Initialize ZO_Base algorithm
    ...
    def _sample_perturbations(self, num_samples, dim):
        """Sample k standard normal distribution perturbation vectors"""
    ...
    def forward(self, x):
        """
        Use zero-order optimization method to optimize adapter parameters
        """
        # get shift vector for calculating shift direction
        shift_vector = self._get_shift_vector()

        # initialize variables to track best loss and output
        ...
        for i, z in enumerate(perturbations):
            ...
            # use formula: ∇f(x;ε) ≈ (f(x+εz) - f(x-εz))/(2ε) * z
        ...
        # average gradient estimate over all perturbations
        ...
        self._apply_perturbation(update, sign=1)  # use optimizer calculated update
        
        # calculate final output and loss
    ...
    def obtain_origin_stat(self, train_loader):
        ...
        # try to load precalculated statistics
        ...
        # prepare quantized model for quick adaptation
