        ...
        for _, m in self.model.vit.named_modules():
            if type(m) == PTQSLBatchingQuantMatMul:
                m._get_padding_parameters(
                    torch.zeros((1,num_layers,197+self.model.num_prompts,head_dim)).cuda(), 
                    torch.zeros((1,num_layers,64,197+self.model.num_prompts)).cuda()
                    )
            elif type(m) == SoSPTQSLBatchingQuantMatMul:
                m._get_padding_parameters(
                    torch.zeros((1,num_layers,197+self.model.num_prompts,197+self.model.num_prompts)).cuda(), 
                    torch.zeros((1,num_layers,197+self.model.num_prompts,head_dim)).cuda()
                    )
